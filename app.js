...
const activateLightBox = (thisElmt) => {
  ...
  // 라이트 박스가 활성화 될 떄 클릭된 이미지 요소의 src 속성 값을 읽어 라이트 박스 이미지 src를 속성값으로 전달
...
// 이미지 요소들을 순회하여 각각을 클릭했을 때 activeLighBox 함수 실행 : 클릭한 대상 요소를 파라미터로 전달
images.forEach((img) => {
...
// 이미지 요소 배열을 순회한다. 첫번쨰 파라미터는 이미지 요소 배열, 두번쨰 파라미터는 boolean 값 - true 일 경우? 뒤에 : 을 기준으로 앞에게 적용되고, false 일 경우 뒤에 로직이 적용된다.
...
function showItems(thisItem) {
  ...
  // 탭 버튼을 클릭했을 때 클릭된 탭의 date-filter 속성값 저장
